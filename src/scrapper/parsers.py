        ...
        if resp.status_code == 200:
            ...
            if main_div:
                ...
                for div in div_list:
                    ...
                    if logo:
                        company = logo['alt']
                    jobs.append({'title': title.text, 'url': domain +
                                 href, 'description': content, 'company': company, 'city_id': None, 'language_id': None})
            else:
            ...
            if not new_jobs:
                ...
                if table:
                    ...
                    for tr in tr_list:
                        ...
                        if div:
                            title = div.find(
                                'p', attrs={'class': 'card-title'})
                            href = title.a['href']
                            content = div.find(
                                'div', attrs={"class": "card-description"}).text
                            company = 'No name'
                    ...
                    errors.append(
                        {'url': url, 'title': 'Table does not exist'})
            else:
...
def dou(url, city=None, language=None):
    ...
    if url:
        ...
        if resp.status_code == 200:
            soup = BS(resp.content, 'html.parser')
            ul = soup.find('ul', attrs={'class': 'lt'})
            ...
            if li_list:
                for li in li_list:
                    ...
                    company = 'No name'
                    c_check_name = div_title.find(
                        'a', attrs={'class': 'company'})
                    if c_check_name:
                        company = c_check_name.text
                    jobs.append({'title': title, 'url': href,
                                 'description': content, 'company': company, 'city_id': None, 'language_id': None})

            else:
                errors.append({'url': url, 'title': 'Li_list does not exist'})
        else:
            errors.append({'url': url, 'title': 'Page do not response'})
    return jobs, errors
