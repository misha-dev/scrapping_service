        ...
        if main_div:
            ...
            for div in div_list:
                ...
                jobs.append({'title': title.text, 'url': domain +
                             href, 'description': content, 'company': company})
        else:
    ...
    if resp.status_code == 200:
        ...
        if li_list:
            for li in li_list:
                ...
                company = 'No name'
                c_check_name = div_title.find('a', attrs={'class': 'company'})
                if c_check_name:
...
def djinni(url):
    ...
    if resp.status_code == 200:
        soup = BS(resp.content, 'html.parser')
        ul = soup.find('ul', attrs={'class': 'list-jobs'})
        li_list = ul.find_all('li')
        if li_list:
            for li in li_list:
                ...
                href = div_title.a['href']
                content = li.find(
                    'div', attrs={'class': 'list-jobs__description'}).p.text
                company = 'No name'
                c_check_name = li.find(
